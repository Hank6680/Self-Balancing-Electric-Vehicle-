...
Based on:
Adeept Self-Balancing Robot Kit Example Code
https://www.adrive.com/public/97GXSs/Adeept_SelfBalancingRobotKit-V1.0.zip
www.adeept.com (accessed November 17, 2022)

Description:
This code provides the baseline control framework for a two-wheeled
self-balancing robot based on an inverted pendulum model. It includes
sensor initialization, calibration routines, and motor control logic
required for balance operation.

Modifications and Extensions:
Modified and extended by YUNXIANG HAN as part of an academic and
engineering project. Changes include parameter tuning, control logic
adjustments, hardware integration updates, and experimental validation
for improved balance performance and system understanding.
...
float desired_distance = 2000; // desired distance to travel
...
float kp = 30*1.1,  kd = 0.58*0.7;                               // pitch angle PD control gains
...
volatile float accumulated_yaw = 0;  // 累积偏航角
volatile float yaw_target = 0;       // 目标偏航角
volatile float yaw_error = 0;        // 偏航角误差
volatile float previous_yaw_error = 0; // 记录前一帧的误差      
