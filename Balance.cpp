...
{
  float speeds = (pulseleft + pulseright);               // overall vehicle speed over 40ms                                                    // (but could be also slightly turning)
  ...
  positions = constrain(positions, -2200,2200); 
//  float output = constrain(output, -250,250); 
  float output = kis * (p0 - positions) + kps * (p0 - speeds_filter);
  extern volatile float accumulated_yaw;
  if(abs(kalmanfilter.angle) > 30 ) 
   {
      positions=0;
   }
//  Serial.print("speed =");Serial.print(speeds);
//  Serial.print("speed_filter =");Serial.print(speeds_filter);
//  Serial.print("position =");Serial.print(positions);
...
// turnoutput = return yaw rotation PD control output
{
  ...
  {
    rotationratio = 0; // minimum rotationratio
    turnspeed = 0.0;
  }
  ...
  turnoutput = yaw_error * kpturn*0.02 - Gyroz * kdturn; 
//  if (abs(yaw_error) < 10) {  
//    turnoutput = 0.2*turnoutput;
//    
//}
  //Serial.print("gro = ");Serial.println(Gyroz);
...
void Balance::distance_control(){
  position_error = target_distance - positions; 
  position_output = Kp_pos * position_error + Ki_pos * sum_position_error - Kd_pos * (position_error - last_position_error);
//  Serial.print("position = ");  Serial.print(positions);
//  Serial.print("error = ");  Serial.print(position_error);
//  Serial.print("sumerror =");Serial.print(sum_position_error);
//  Serial.print("difference =");Serial.print(position_error - last_position_error);
//  Serial.print("output =");Serial.println(position_output);
  
  
  last_position_error = position_error;
  sum_position_error += position_error; // 累计误差（积分项）
}
...
// PinPWMB = right motor pwm command pin
{
  ...
  pwm1 = -angleoutput - speedoutput - rotationoutput + 0*position_output; //Left motor PWM output value, rotation command opposite 
                                                      // to each motor
  pwm2 = -angleoutput - speedoutput + rotationoutput + 0*position_output; //Right motor PWM output value

//  Serial.print("postion = ");  Serial.print(positions);
//  Serial.print("-angle =");Serial.print(-angleoutput);
//  Serial.print("-speed =");Serial.print(-speedoutput);
//  Serial.print("-rotation =");Serial.print(-rotationoutput);
//  Serial.print("-position_output =");Serial.println(-position_output);
